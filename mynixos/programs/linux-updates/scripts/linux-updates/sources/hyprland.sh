...
fetch_latest() {
    local response
    local latest_url="https://api.github.com/repos/hyprwm/Hyprland/releases/latest"
    local tags_url="https://api.github.com/repos/hyprwm/Hyprland/tags?per_page=1"
    ...
    if [[ -n "$response" ]] && echo "$response" | jq -e '.id?' >/dev/null 2>&1; then
        ...
        NEWS_TITLE=$(echo "$response" | jq -r '.name // .tag_name // "New Hyprland release"')
    ...
    response=$(curl \
        ...
        -H "X-GitHub-Api-Version: 2022-11-28" \
        "$tags_url") || {
        echo "Failed to download Hyprland tags" >&2
        return 1
    }

    if ! echo "$response" | jq -e 'type == "array" and length > 0' >/dev/null 2>&1; then
        echo "No Hyprland tags found" >&2
        return 1
    fi
    ...
    NEWS_ID="$tag_sha"
    NEWS_TITLE="Hyprland ${tag_name}"
    NEWS_URL="https://github.com/hyprwm/Hyprland/releases/tag/${tag_name}"
    NEWS_SOURCE="github_tag"
    NEWS_CONTENT="New Hyprland tag detected: ${tag_name}"
