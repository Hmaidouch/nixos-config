...
fetch_latest() {
    local response
    local latest_url="https://api.github.com/repos/JetBrains/kotlin/releases/latest"
    local tags_url="https://api.github.com/repos/JetBrains/kotlin/tags?per_page=1"
    ...
    if [[ -n "$response" ]] && echo "$response" | jq -e '.id?' >/dev/null 2>&1; then
        ...
        NEWS_TITLE=$(echo "$response" | jq -r '.name // .tag_name // "New Kotlin release"')
    ...
    response=$(curl \
        ...
        -H "X-GitHub-Api-Version: 2022-11-28" \
        "$tags_url") || {
        echo "Failed to download Kotlin tags" >&2
        return 1
    }

    if ! echo "$response" | jq -e 'type == "array" and length > 0' >/dev/null 2>&1; then
        echo "No Kotlin tags found" >&2
        return 1
    fi
    ...
    NEWS_ID="$tag_sha"
    NEWS_TITLE="Kotlin ${tag_name}"
    NEWS_URL="https://github.com/JetBrains/kotlin/releases/tag/${tag_name}"
    NEWS_SOURCE="github_tag"
    NEWS_CONTENT="New Kotlin tag detected: ${tag_name}"
