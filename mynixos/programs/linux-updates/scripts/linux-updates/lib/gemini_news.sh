...
${url}"
    ...
    if [[ -z "${content// }" ]]; then
        ...
        local extra_rules=""
    if [[ "$project" == "NixOS" || "$source" == "announcement" || "$source" == "announcements_rss" ]]; then
        extra_rules=$(cat <<'EOF'
تعليمات إضافية خاصة بأخبار NixOS:
- إذا كان الخبر إعلان إصدار جديد من NixOS، استخرج النقاط المهمة من النص بدل الاكتفاء بقول "تم إصدار نسخة جديدة".
- ركز خصوصًا على:
  1) مدة الدعم أو نهاية دعم النسخة السابقة
  2) أهم Highlights المذكورة في الخبر
  3) تحديثات GNOME / GCC / systemd stage 1 / تغييرات المنصات إن وُجدت
  4) أرقام مهمة مثل عدد الحزم الجديدة أو المحدثة إذا كانت موجودة
- لا تقل "لا توجد تفاصيل" إذا كان النص يحتوي على تفاصيل فعلية.
- إذا كان المحتوى يحتوي على قسم Highlights أو lists أو نقاط تعداد، فاستخرج أهم 3 إلى 5 نقاط منها.
EOF
)
    fi
...
- لا تتجاهل التفاصيل الموجودة في المحتوى الخام إذا كانت واضحة.
- لا تقل "لا توجد معلومات كافية" إذا كان النص يحتوي على معلومات صريحة عن التغييرات أو المزايا.
...
• نقطة ثانية توضح أهم تغيير أو ميزة
• نقطة ثالثة تذكر لماذا قد يهم المستخدم
• أضف نقطة رابعة أو خامسة إذا كانت هناك تفاصيل تقنية مهمة فعلًا
